...
function prepare_partitions() {
	...
	# 	[[ $FIND_LOOP_CYCLES -gt 20 ]] && exit_with_error "Unable to find free loop device"
	...
	LOOP=$(losetup --find --show --partscan "${SDCARD}".raw) || exit_with_error "Failed to set up loop device"
